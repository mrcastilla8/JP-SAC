...
class ReconciliationRulesEngine:
    ...
    def reconcile_investigador(
        self, current: Optional[Dict[str, Any]], incoming: InvestigadorInput, fuente: str
    ...
    def reconcile_proyecto(
        self, current: Optional[Dict[str, Any]], incoming: ProyectoInput, fuente: str
    ) -> Tuple[Dict[str, Any], bool, str]:
        incoming_dict = incoming.model_dump(exclude_unset=True, exclude_none=True)
        if not current:
            return incoming_dict, False, ""
    ...
    def reconcile_publicacion(
        self, current: Optional[Dict[str, Any]], incoming: PublicacionInput, fuente: str
    ) -> Tuple[Dict[str, Any], bool, str]:
        incoming_dict = incoming.model_dump(exclude_unset=True, exclude_none=True)
        if not current:
            return incoming_dict, False, ""
    ...
    def reconcile_asesor_tesis(
        self, padron_investigadores: Dict[str, str], incoming: AsesorTesisInput
    ) -> Tuple[Dict[str, Any], bool, str]:
        incoming_dict = incoming.model_dump(exclude_unset=True, exclude_none=True)
        ...
        if dni_encontrado:
            ...
            return (
                incoming_dict,
                True,
                "No se pudo hacer match del asesor de tesis por nombre con la confianza requerida.",
            )
