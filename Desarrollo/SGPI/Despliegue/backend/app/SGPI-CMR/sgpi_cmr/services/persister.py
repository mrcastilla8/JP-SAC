...
class ReconciliationPersister:
    async def persist_quarantine(
        ...
        conflicto_serializable = {
            k: v.isoformat() if isinstance(v, (datetime.datetime, datetime.date)) else v for k, v in conflicto.items()
        }
    ...
    async def persist_resolved(
        self, db: AsyncSession, entidad: str, llave_pk: str, merged_data: Dict[str, Any], fuente_ganadora: str
    ) -> None:
        """
        ...
        try:
            ...
            if entidad == "investigador":
                ...
                if existing:
                    await db.execute(
                        update(Investigador)
                        .where(Investigador.dni == llave_pk)
                        .values(**merged_data, updated_at=func.now())
                    )
                    is_update = True
                else:
                ...
                if result.scalars().first():
                    await db.execute(
                        update(Proyecto)
                        .where(Proyecto.codigo_proyecto == llave_pk)
                        .values(**merged_data, updated_at=func.now())
                    )
                    is_update = True
                else:
                ...
                if existing:
                    ...
                    await db.execute(
                        update(Publicacion)
                        .where(Publicacion.id_publicacion == existing.id_publicacion)
                        .values(**merged_data)
                    )
                    is_update = True
                else:
