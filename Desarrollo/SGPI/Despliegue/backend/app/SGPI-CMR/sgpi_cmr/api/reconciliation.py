...
from sgpi_cmr.schemas.incoming import (
    BulkInvestigadorPayload,
    BulkProyectoPayload,
    BulkPublicacionPayload,
    BulkAsesorTesisPayload,
)
...
async def reconcile_investigadores_bulk(
    payload: BulkInvestigadorPayload, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
    ...
    for registro in payload.registros:
        ...
        if requires_quarantine:
            ...
            await persister.persist_resolved(
                db, entidad="investigador", llave_pk=registro.dni, merged_data=merged, fuente_ganadora=fuente
            )
...
async def reconcile_proyectos_bulk(
    payload: BulkProyectoPayload, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
    fuente = payload.fuente_origen
    stats = {"procesados": 0, "resueltos": 0, "cuarentena": 0}
    ...
    for registro in payload.registros:
        ...
        if requires_quarantine:
            ...
            await persister.persist_quarantine(
                db,
                entidad="proyecto",
                llave_sugerida=registro.codigo_proyecto,
                fuentes=[fuente],
                conflicto=merged,
                motivo=reason,
            )
            ...
            stats["resueltos"] += 1
            await persister.persist_resolved(
                db, entidad="proyecto", llave_pk=registro.codigo_proyecto, merged_data=merged, fuente_ganadora=fuente
            )
...
async def reconcile_publicaciones_bulk(
    payload: BulkPublicacionPayload, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
    fuente = payload.fuente_origen
    stats = {"procesados": 0, "resueltos": 0, "cuarentena": 0}
    ...
    for registro in payload.registros:
        ...
        if requires_quarantine:
            stats["cuarentena"] += 1
            await persister.persist_quarantine(
                db, entidad="publicacion", llave_sugerida=llave, fuentes=[fuente], conflicto=merged, motivo=reason
            )
        else:
            stats["resueltos"] += 1
            await persister.persist_resolved(
                db, entidad="publicacion", llave_pk=llave, merged_data=merged, fuente_ganadora=fuente
            )
...
@router.post(
    "/bulk/theses_advisors", summary="Reconciliar asesores de tesis (Cybertesis)", status_code=status.HTTP_202_ACCEPTED
)
async def reconcile_asesores_tesis(
    payload: BulkAsesorTesisPayload, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
    fuente = payload.fuente_origen
    stats = {"procesados": 0, "resueltos": 0, "cuarentena": 0}
    ...
    for registro in payload.registros:
        ...
        if requires_quarantine:
            ...
            await persister.persist_quarantine(
                db,
                entidad="tesis",
                llave_sugerida=registro.url_cybertesis,
                fuentes=[fuente],
                conflicto=merged,
                motivo=reason,
            )
            ...
            stats["resueltos"] += 1
            await persister.persist_resolved(
                db, entidad="tesis", llave_pk=registro.url_cybertesis, merged_data=merged, fuente_ganadora=fuente
            )
...
async def get_quarantine_items(db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)):
...
async def resolve_quarantine_item(
    id_pendiente: int, action: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
