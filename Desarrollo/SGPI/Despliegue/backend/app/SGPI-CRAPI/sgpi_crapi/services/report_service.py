...
# Ponderaciones base
HORAS_PROYECTO_ROLES = {"Responsable": 10.0, "Co-Investigador": 6.0, "Colaborador": 3.0}
...
async def _calculate_workloads_batch(db: AsyncSession, params: ReportParams, investigadores: list):
    ...
    stmt_proj = (
        select(Proyecto, InvestigadorProyecto)
        .join(InvestigadorProyecto, Proyecto.codigo_proyecto == InvestigadorProyecto.codigo_proyecto)
        .where(
            InvestigadorProyecto.dni_investigador.in_(dnis), Proyecto.estado_proyecto.in_(["Aprobado", "En ejecución"])
        )
    )
    ...
    stmt_tesis = select(Tesis).where(
        Tesis.dni_asesor.in_(dnis), (Tesis.anio_publicacion == anio_tesis) | (Tesis.anio_publicacion.is_(None))
    )
...
async def generate_workload_report(db: AsyncSession, params: ReportParams) -> WorkloadReportResponse:
    ...
    return WorkloadReportResponse(
        parametros=params, total_investigadores=len(workload_details), investigadores=workload_details
    )
...
async def generate_active_projects_report(db: AsyncSession, params: ReportParams) -> ActiveProjectsResponse:
    ...
    if params.grupo_investigacion:
        stmt = stmt.join(GrupoInvestigacion, Proyecto.id_grupo == GrupoInvestigacion.id_grupo).where(
            GrupoInvestigacion.nombre_grupo == params.grupo_investigacion
        )

    if params.departamento_academico:
        subq = (
            select(InvestigadorProyecto.codigo_proyecto)
            .join(Investigador, Investigador.dni == InvestigadorProyecto.dni_investigador)
    ...
    if not proyectos:
        return ActiveProjectsResponse(parametros=params, total_proyectos=0, presupuesto_total=0.0, proyectos=[])

    # Obtener integrantes
    codigos_proyectos = [p.codigo_proyecto for p in proyectos]
    stmt_miembros = (
        select(InvestigadorProyecto, Investigador)
        .join(Investigador, Investigador.dni == InvestigadorProyecto.dni_investigador)
    ...
    for ip, inv in miembros_data:
        ...
        miembros_por_proyecto[ip.codigo_proyecto].append(
            ActiveProjectMember(dni=inv.dni, nombres=inv.nombres, apellidos=inv.apellidos, rol=ip.condicion_rol)
        )
...
async def generate_scientific_production_report(db: AsyncSession, params: ReportParams) -> ScientificProductionResponse:
    ...
    if params.grupo_investigacion:
        stmt_pub = stmt_pub.join(GrupoInvestigacion, Publicacion.id_grupo == GrupoInvestigacion.id_grupo).where(
            GrupoInvestigacion.nombre_grupo == params.grupo_investigacion
        )

    if params.departamento_academico:
        subq = (
            select(InvestigadorPublicacion.id_publicacion)
            .join(Investigador, Investigador.dni == InvestigadorPublicacion.dni_investigador)
            .where(Investigador.departamento_academico == params.departamento_academico)
        )
    ...
    if publicaciones:
        ...
        stmt_autores = (
            select(InvestigadorPublicacion, Investigador)
            .join(Investigador, Investigador.dni == InvestigadorPublicacion.dni_investigador)
        ...
        for ip, inv in autores_data:
            ...
            autores_por_pub[ip.id_publicacion].append(
                ScientificAuthor(
                    dni=inv.dni, nombres=inv.nombres, apellidos=inv.apellidos, filiacion_unmsm=ip.filiacion_unmsm
                )
            )

        for p in publicaciones:
            publicaciones_detalle.append(
                ScientificProductionDetail(
    ...
    if params.departamento_academico:
        stmt_tesis = stmt_tesis.join(Investigador, Investigador.dni == Tesis.dni_asesor).where(
            Investigador.departamento_academico == params.departamento_academico
        )
...
async def generate_general_summary_report(db: AsyncSession, params: ReportParams) -> GeneralSummaryResponse:
    ...
    # 4. Categoría RENACYT
    stmt_renacyt = select(Investigador.categoria_renacyt, func.count(Investigador.dni)).group_by(
        Investigador.categoria_renacyt
    )
    ...
    total_grupos = (
        await db.execute(
            select(func.count(GrupoInvestigacion.codigo_grupo)).where(GrupoInvestigacion.estado_grupo == "Activo")
        )
    ).scalar() or 0

    # 6. Investigadores SM y Deudas
    total_sm = (
        await db.execute(select(func.count(Investigador.dni)).where(Investigador.investigador_sm.is_(True)))
    ).scalar() or 0
    deuda_pi = (
        await db.execute(select(func.count(Investigador.dni)).where(Investigador.tiene_deuda_pi.is_(True)))
    ).scalar() or 0
    deuda_gi = (
        await db.execute(select(func.count(Investigador.dni)).where(Investigador.tiene_deuda_gi.is_(True)))
    ).scalar() or 0

    # 7. Convocatorias
    res_convocatorias = (
        (await db.execute(select(Convocatoria.fecha_cierre).where(Convocatoria.estado_convocatoria == "Abierta")))
        .scalars()
        .all()
    )
