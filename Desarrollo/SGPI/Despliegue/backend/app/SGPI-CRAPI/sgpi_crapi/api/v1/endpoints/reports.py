...
from sgpi_crapi.schemas.report_schemas import ReportParams, SnapshotPOICreate, SnapshotPOISummary, SnapshotPOIResponse
...
async def snapshot_report(params: ReportParams, db: AsyncSession = Depends(get_db)):
    """
...
async def list_snapshots(
    skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db), current_user: dict = Depends(require_staff)
):
    """
...
async def get_catalogs(db: AsyncSession = Depends(get_db)):
    ...
    dept_stmt = (
        select(Investigador.departamento_academico).filter(Investigador.departamento_academico.isnot(None)).distinct()
    )
    ...
    return {"departamentos": sorted(departamentos), "grupos": sorted(grupos)}
