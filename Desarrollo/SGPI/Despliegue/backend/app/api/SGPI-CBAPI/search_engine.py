...
class SearchEngine:
    ...
    def _deduce_source_proyecto(proj: Proyecto) -> str:
        ...
        if proj.codigo_proyecto and (
            proj.codigo_proyecto.startswith("F")
            or proj.codigo_proyecto.startswith("A")
            or len(proj.codigo_proyecto) > 6
        ):
            return "RAIS"
    ...
    async def search(self, db: AsyncSession, req: SearchRequest) -> SearchResponse:
        ...
        categories_to_query = (
            req.category if req.category else ["Investigador", "Proyecto", "Grupo", "Publicacion", "Tesis"]
        )
                ...
                results.append(
                    UnifiedSearchItem(
                        ...
                        details={
                            ...
                            "presupuesto_asignado": float(proj.presupuesto_asignado)
                            if proj.presupuesto_asignado
                            else 0.0,
        ...
        return SearchResponse(
            total_results=total_results, page=page, limit=limit, total_pages=total_pages, results=paginated_results
        )
