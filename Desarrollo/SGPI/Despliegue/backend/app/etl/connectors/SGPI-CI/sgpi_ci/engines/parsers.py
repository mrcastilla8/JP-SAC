...
class ProyectosParser:
    ...
    def parse(self, file_path: str) -> Dict[str, List[dict]]:
        ...
        for _, row in df.iterrows():
            ...
            rol = "Miembro"
            if docente_raw.startswith("R_"):
                rol = "Responsable"
            elif docente_raw.startswith("C_"):
                rol = "Corresponsable"
            elif docente_raw.startswith("A_"):
                rol = "Asesor"
        ...
        if "Proyectos con Financiamiento" in xl.sheet_names:
            ...
            for _, row in df_p.iterrows():
                ...
                if resp and resp != "nan":
                    miembros.append(f"R_{resp}")
                if coresp and coresp != "nan":
                    miembros.append(f"C_{coresp}")
                ...
                result["publicaciones"].append(
                    {
                        ...
                        "docente_nombre": clean_prefix_and_title(
                            pd.Series([str(row.get("Primer autor Filiación", "")).split("\n")[0]])
                        ).iloc[0],
                    }
                )
...
class GIDocentesParser:
    ...
    def parse(self, file_path: str) -> Dict[str, List[dict]]:
        ...
        for _, row in df.iterrows():
            ...
            if not nombre_gi or nombre_gi == "nan":
                continue
