...
def main():
    ...
    group.add_argument(
        ...
        "--institution",
        type=str,
        help="Query researchers by self-declared CTI VITAE main institution (ilike match).",
    )

    # Pagination
    parser.add_argument("-p", "--page", type=int, default=1, help="Page number to fetch (default: 1).")
    parser.add_argument(
        "-l", "--limit", type=int, default=10, help="Number of records to fetch per page (default: 10)."
    )
    ...
    parser.add_argument(
        ...
        "--format",
        type=str,
        choices=["json", "json-compact", "csv"],
        default="json",
        help="Output format to display results (default: json).",
    )
    parser.add_argument(
        "-out", "--output", type=str, help="Write output results directly to a local file instead of stdout."
    )
    ...
    parser.add_argument(
        "--delay", type=float, default=1.0, help="Inter-request rate limit delay in seconds (default: 1.0)."
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    connector = RenacytConnector(verify_ssl=args.ssl_verify, rate_limit_delay=args.delay, max_retries=3)
    ...
    async def run_query():
        ...
        # 5. Query by Last Name (returns search list)
            ...
            results = await connector.search_by_lastname(
                args.lastname, page=args.page, page_size=args.limit, normalize=True
            )
        ...
        # 6. Query by Institution (returns search list)
            ...
            results = await connector.search_by_institution(
                args.institution, page=args.page, page_size=args.limit, normalize=True
            )
