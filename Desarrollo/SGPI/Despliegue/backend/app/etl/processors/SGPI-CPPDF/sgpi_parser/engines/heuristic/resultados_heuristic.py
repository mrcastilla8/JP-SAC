...
class HeuristicResultadosParser(BaseParser):
    ...
    def parse(self, pdf_path: str) -> ResultadosConcurso:
            ...
            for page in pdf.pages:
                ...
                for table_obj in tables:
                    ...
                    for idx, col_name in enumerate(header):
                        ...
                        if (
                            col_clean in ["n°", "n", "nro", "orden", "puesto"]
                            or "merito" in col_clean
                            or "mérito" in col_clean
                        ):
                            ...
                            "puntaje" in col_clean
                            or "puntos" in col_clean
                            or "nota" in col_clean
                            or "total" in col_clean
                        ):
                            col_map["puntaje"] = idx
                    ...
                    for row in table_rows[1:]:
                        ...
                        if any(
                            clean_text(str(cell)).lower() in ["n°", "tïtulo del proyecto", "responsable"]
                            for cell in row
                        ):
                            continue
        ...
        metadata = MetadataResultados(programa_nombre=programa_nombre, anio_academico=detected_year)

        return ResultadosConcurso(
            tipo_documento="resultados", metadata=metadata, proyectos_aprobados=proyectos_aprobados
        )
