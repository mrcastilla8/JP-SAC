...
class HeuristicCronogramaParser(BaseParser):
    ...
    def parse(self, pdf_path: str) -> Cronograma:
        ...
        for idx, line in enumerate(text_lines):
            ...
            if (
                "fecha" in lower_line
                or "detalle" in lower_line
                or "actividad" in lower_line
                or "dependencia" in lower_line
            ):
                continue
            ...
            if "programa para" in lower_line or "programa de" in lower_line:
                ...
                "programa" in lower_line
                or "convocatoria" in lower_line
                or "concurso" in lower_line
                or "picv" in lower_line
            ):
                score += 100
        ...
        if best_line_idx != -1 and best_score > 0:
            ...
            if best_line_idx + 1 < len(text_lines):
                ...
                if (
                    ...
                    and not any(
                        exclude in next_lower
                        for exclude in ["inicio", "término", "fecha", "detalle", "cronograma", "dependencia"]
                    )
                    and (next_line.isupper() or any(char.isdigit() for char in next_line) or "(" in next_line)
                ):
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    ...
                    for idx, row in enumerate(table[:3]):
                        ...
                        if any(
                            "actividad" in cell or "detalle" in cell or "concepto" in cell for cell in row_lower
                        ) or any("fecha" in cell or "plazo" in cell or "cronograma" in cell for cell in row_lower):
                            header_row_idx = idx
                            header = row
                            header_found = True
                            break
                    ...
                    for row in table[header_row_idx + 1 :]:
                        ...
                        if not actividad_desc or actividad_desc.lower() in [
                            "detalle",
                            "actividad",
                            "actividades",
                            "concepto",
        ...
        metadata = MetadataCronograma(programa_nombre=programa_nombre, anio_academico=detected_year)

        return Cronograma(tipo_documento="cronograma", metadata=metadata, actividades=actividades)
