...
from sgpi_capirestc.schemas.domain_schemas import UsuarioCreate, UsuarioUpdate, UsuarioResponse
...
async def create_usuario(
    obj_in: UsuarioCreate, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    # UUID will be auto-generated by DB or Supabase in a real flow,
    # but for now we rely on the DB default gen_random_uuid
...
async def get_usuario(
    id_usuario: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    try:
