...
from app.models.domain import Publicacion, Tesis, GrupoInvestigacion
...
async def list_producciones(
    ...
    if tipo in ["todos", "articulo"]:
        ...
        if buscar:
            term = f"%{buscar}%"
            filters_pub.append(or_(Publicacion.titulo_articulo.ilike(term), Publicacion.doi_codigo.ilike(term)))
        if filters_pub:
        ...
        for p, g in res_pub.all():
            ...
            if g:
                grupo_vinculado = GrupoInvestigacionResumen(
                    id=g.id_grupo, nombre=g.nombre_grupo, siglas=g.siglas, facultad=g.facultad
                )
            resultados.append(
                RegistroProduccionResponse(
    ...
    if tipo in ["todos", "tesis"]:
        ...
        if buscar:
            term = f"%{buscar}%"
            filters_tes.append(or_(Tesis.titulo_tesis.ilike(term), Tesis.autor_estudiante_texto.ilike(term)))
        if filters_tes:
...
async def get_produccion(id: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    if id.startswith("pub-"):
        pub_id = int(id[4:])
        stmt = (
            select(Publicacion, GrupoInvestigacion)
            .outerjoin(GrupoInvestigacion, Publicacion.id_grupo == GrupoInvestigacion.id_grupo)
...
async def confirmar_produccion(
    id: str,
    ...
    if id.startswith("pub-"):
        ...
        if payload.doi:
            p.doi_codigo = payload.doi
        if payload.issn:
            p.issn = payload.issn
        if payload.volNum:
            p.volumen = payload.volNum
        if payload.revista:
            p.nombre_revista = payload.revista
        if payload.cuartil:
            p.cuartil_impacto = payload.cuartil

        await db.commit()
        await log_audit_event(
            db, "UPDATE", "publicacion", str(pub_id), None, {"estado_validacion": "Validado"}, current_user.get("sub")
        )
        ...
        await db.commit()
        await log_audit_event(
            db, "UPDATE", "tesis", url, None, {"estado_validacion": "Validado"}, current_user.get("sub")
        )
        return await get_produccion(id, db, current_user)
...
async def list_grupos_investigacion(query: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    stmt = select(GrupoInvestigacion).where(GrupoInvestigacion.estado_grupo == "Activo")
    if query:
        term = f"%{query}%"
        stmt = stmt.where(or_(GrupoInvestigacion.nombre_grupo.ilike(term), GrupoInvestigacion.siglas.ilike(term)))
    res = await db.execute(stmt)
    resultados = []
    for g in res.scalars().all():
        resultados.append(
            GrupoInvestigacionResumen(id=g.id_grupo, nombre=g.nombre_grupo, siglas=g.siglas, facultad=g.facultad)
        )
    return resultados
