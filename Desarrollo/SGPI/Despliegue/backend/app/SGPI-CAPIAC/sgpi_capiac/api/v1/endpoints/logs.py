...
async def read_system_logs(
    lines: int = Query(100, ge=1, le=1000), level: Optional[str] = None, current_user: dict = Depends(require_admin)
) -> Any:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error leyendo logs del sistema: {str(exc)}")


@router.get("/system/download")
async def download_system_logs(current_user: dict = Depends(require_admin)) -> Any:
    """
    ...
    log_file_path = settings.LOG_FILE_PATH
    if not os.path.exists(log_file_path):
        raise HTTPException(status_code=404, detail="Archivo de logs de diagnóstico no encontrado en el servidor.")
